...
uint32_t kvarena_delcnt(KVArena *kva_p) {
    return (!kva_p) ? 0 : kva_p->delcnt;
}
...
KVArena *create_kvarena(
    ...
    _dbg_log_msg("#4 set struct fields");
    kva_p->align            = align;
    kva_p->entrycnt         = 0;
    kva_p->delcnt           = 0;
    ...
    return kva_p;
failed:
    _dbg_log_msg("failed: kva_p->~KVArena();");
...
int32_t kvarena_push(
    ...
    _dbg_log_msg("#1 check_if_full");
    if (kva_p->entrycnt >= kva_p->entrycap) goto failed_ret;

    _dbg_log_msg("#2 assert_intrnl_state");
    ...
    _dbg_log_msg("#5 fill_entry_fields");
...
int kvarena_is_entry_valid(
    const KVArena  *kva_p,
    uint32_t        ent_idx
) {
    if (!kva_p || !kva_p->entrytbl || !kva_p->data_buf) goto failed_ret;
    if (ent_idx >= kva_p->entrycnt || !kva_p->entrycnt) goto failed_ret;
...
int32_t kvarena_get(
    ...
    _dbg_log_msg("#1 assert_intrnl_state");
    _kva_assert_intrnl_state(kva_p);

    _dbg_log_msg("#2 get_ent");
...
int kvarena_pop(KVArena *kva_p) {
    ...
    _dbg_print("old_back_off: %u; _old_back_off: %u", old_back_off, _old_back_off);
...
int kvarena_compact(KVArena **kva_pp) {
    ...
    _dbg_log_msg("#4 destroy_kvarena");
...
KVArenaIterator kvarena_iterator_begin(
    const KVArena  *kva_p
) {
    return (!kva_p || !kva_p->entrycnt)
        ? NULL : kva_p->entrytbl;
}

KVArenaIterator kvarena_iterator_end(
    const KVArena  *kva_p
) {
    return (!kva_p || !kva_p->entrycnt)
        ? NULL : kva_p->entrytbl + kva_p->entrycnt;
}

KVArenaIterator kvarena_iterator_next(
    const KVArena      *kva_p,
    KVArenaIterator     it
) {
    (void)kva_p;
...
KVArenaIterator kvarena_iterator_rbegin(
    const KVArena  *kva_p
) {
    return (!kva_p || !kva_p->entrycnt)
        ? NULL : kva_p->entrytbl + kva_p->entrycnt - 1;
}

KVArenaIterator kvarena_iterator_rend(
    const KVArena  *kva_p
) {
    return (!kva_p || !kva_p->entrycnt)
        ? NULL : kva_p->entrytbl - 1;
}

KVArenaIterator kvarena_iterator_rnext(
    const KVArena      *kva_p,
    KVArenaIterator     it
) {
    (void)kva_p;
